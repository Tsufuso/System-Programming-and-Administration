...
list_tasks() {
    ...
    # List completed tasks for the current day
    echo "Completed tasks for $current_date:"
    awk -F'|' -v date="$current_date" '$5 ~ date && $6 == "Y" { print $0 }' "$todo_file"

    # List uncompleted tasks for the current day
    echo "Uncompleted tasks for $current_date:"
    awk -F'|' -v date="$current_date" '$5 ~ date && $6 == "N" { print $0 }' "$todo_file"
}
